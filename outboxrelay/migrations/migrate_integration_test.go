...
func TestApplyVerifyAndConcurrentRunners(t *testing.T) {
	...
	var original string
	if err := db.QueryRow(context.Background(), `SELECT checksum FROM outbox_relay_schema_migrations WHERE version='001_outbox_leases.sql'`).Scan(&original); err != nil {
		t.Fatal(err)
	}
	if _, err := db.Exec(context.Background(), `UPDATE outbox_relay_schema_migrations SET checksum='invalid' WHERE version='001_outbox_leases.sql'`); err != nil {
		t.Fatal(err)
	}
	defer func() {
		_, _ = db.Exec(context.Background(), `UPDATE outbox_relay_schema_migrations SET checksum=$1 WHERE version='001_outbox_leases.sql'`, original)
	}()
	if err := Verify(context.Background(), db); err == nil {
	...
	if _, err := db.Exec(context.Background(), `UPDATE outbox_relay_schema_migrations SET checksum=$1 WHERE version='001_outbox_leases.sql'`, original); err != nil {
		t.Fatal(err)
	}
