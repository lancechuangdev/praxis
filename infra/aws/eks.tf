...
resource "aws_eks_cluster" "this" {
  ...
  lifecycle {
    precondition {
      condition     = var.eks_admin_principal_arn != null && can(regex("^arn:[^:]+:iam::[0-9]{12}:role/.+", var.eks_admin_principal_arn))
      error_message = "Set eks_admin_principal_arn to a dedicated IAM role ARN before enabling EKS."
    }
